...
export const addNewTask = (newTask, wbsId, pageLoadTime) => async (dispatch) => {
  try {
    ...
    if (Date.parse(wbs.data.modifiedDatetime) > pageLoadTime) {
      ...
      const userIds = res.data.resources.map(resource => resource.userID);
      await createOrUpdateTaskNotificationHTTP(res.data._id, {}, userIds);
    }
  } catch (error) {
    dispatch(setAddTaskError('error'));
  }
};
