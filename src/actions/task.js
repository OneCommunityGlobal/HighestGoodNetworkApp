...
export const updateNums = updatedList => {
  toast.info('updated list', updatedList);
  return {
...
export const deleteTaskNotification = (userId, taskId, taskNotificationId) => async dispatch => {
  try {
...
export const addNewTask = (newTask, wbsId, pageLoadTime) => async dispatch => {
  let status = 200;
  let task = {};
  ...
  try {
    ...
    task = res.data;
    status = res.status;
    dispatch(postNewTask(task, status));
...
export const updateTask = (taskId, updatedTask, hasPermission, prevTask) => async (
  dispatch,
  getState,
) => {
  let status = 200;
