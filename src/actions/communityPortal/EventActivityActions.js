...
const fetchEvents = async (token, queryParams) => {
  const url = queryParams ? `${ENDPOINTS.EVENTS}?${queryParams}` : ENDPOINTS.EVENTS;
...
export const fetchEventDetails = (token, queryParams) => async dispatch => {
  ...
  try {
    const data = await fetchEvents(token, queryParams);
