...
export const getDashboardDataAI = () => {
  ...
  return async dispatch => {
    return DashboardDataAIPromise.then(res => {
...
export const updateDashboardData = textPrompt => {
  ...
  return async dispatch => {
    await axios.put(ENDPOINTS.AI_PROMPT(), updatedData).then(dispatch(updateAIPrompt(textPrompt)));
  };
};
// ===========================================================================
...
export const updateCopiedPromptDate = userId => {
  return async dispatch => {
    await axios.put(ENDPOINTS.COPIED_AI_PROMPT(userId)).then(dispatch(updateCopiedPrompt(userId)));
  };
};
...
export const getCopiedDateOfPrompt = userId => {
  ...
  return async dispatch => {
    return CopiedPromptDate.then(res => {
