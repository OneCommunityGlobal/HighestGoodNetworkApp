...
export const addSummaryRecipient = (userid) => {
  ...
  return async (dispatch) => {
    try {
      ...
      // Replace console.log with dispatching an error action or another logging mechanism if available
      dispatch(authorizeWeeklySummariesReportError(error.message || 'An error occurred'));
...
export const getSummaryRecipients = () => {
  ...
  return async (dispatch) => {
    try {
      ...
      // Replace console.log with dispatching an error action or another logging mechanism if available
      dispatch(getRecepientsError(error.message || 'An error occurred'));
      return Promise.reject(error); // Ensures a consistent return
    }
  };
};
