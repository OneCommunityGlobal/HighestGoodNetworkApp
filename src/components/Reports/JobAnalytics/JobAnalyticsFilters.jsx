...
const JobAnalyticsFilters = ({ filters, setFilters }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const withAll = ['All', ...Array.from(new Set(arr)).sort()];
        if (alive) {
  ...
  const onChange = (e) => {
    ...
    if (name === 'dateMode') {
      ...
      return setFilters(prev => ({ ...prev, dateMode: 'Custom' }));
    }

    if (name === 'granularity') {
      if (filters.dateMode === 'All') {
        return setFilters(prev => ({ ...prev, granularity: 'totals' }));
      }
    }
  ...
  const wrap = { display: 'flex', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.25rem' };
    ...
    <div style={wrap}>
      ...
      {filters.dateMode === 'Custom' && (
        <>
          <label style={col}>
            Start Date:
            <input type="date" name="startDate" value={filters.startDate} onChange={onChange} style={inputStyle} />
          </label>
          <label style={col}>
            End Date:
            <input type="date" name="endDate" value={filters.endDate} onChange={onChange} style={inputStyle} />
          </label>
        </>
      )}
      ...
      <label style={col}>
        ...
        {nonTotalsDisabled && (
          <small style={{ opacity: 0.7, marginTop: 4, color: isDark ? '#aaa' : '#555' }}>
            Weekly/Monthly/Annually require a custom date range.
          </small>
        )}
      </label>
    </div>
