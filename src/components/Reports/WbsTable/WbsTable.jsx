...
export function WbsTable({ wbs, skip, take, match, canViewWBS, darkMode }) {
    ...
    WbsList = wbs.WBSItems.slice(skip, skip + take).map((item, index) => (
      ...
      >
        ...
        <div className={styles['projects__active--input']}>
          {item.isActive ? (
            <div className={styles.isActive} data-testid="wbs-active-icon">
              <i className="fa fa-circle" aria-hidden="true" />
            </div>
          ) : (
            <div className={styles.isNotActive} data-testid="wbs-inactive-icon">
  ...
  return (
    <div className={`wbs-table ${darkMode ? 'text-light' : ''}`}>
      <h5 style={{ marginBottom: '2.125rem' }} className={styles['wbs-table-title']}>
        WBS
      </h5>
