    ...
    PeopleList = props.userProfiles
      ...
      .map((person, index) => (
        <tr className="teams__tr" id={`tr_${person._id}`} key={person._id}>
          ...
          <td style={{width: '110px', textAlign:"center"}}>{moment(person.createdDate).format('MMM-DD-YY')}</td>
          <td style={{width: '110px', textAlign:"center"}}>{moment(person.endDate).format('MMM-DD-YY') || 'N/A'}</td>
          {/* <td>
