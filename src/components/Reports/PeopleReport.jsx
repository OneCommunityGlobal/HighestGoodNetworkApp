...
class PeopleReport extends Component {
  ...
  render() {
    ...
    const StatusOptions = (props) => {
      let allStatus = [...Array.from(new Set(props.get_tasks.map(item => item.status))).sort()];
    ...
    const UserOptions = (props) => {
      let users = [];
      props.userTask.map((task, index) => task.resources.map(resource => users.push(resource.name)));
