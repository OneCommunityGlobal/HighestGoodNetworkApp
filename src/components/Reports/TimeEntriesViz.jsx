...
const TimeEntriesViz = ({timeEntries, fromDate, toDate}) =>{
  ...
  function displayGraph (logs, maxHoursCount, totalHours) {
        ...
        var tooltipEl = function(d) {
          return (
            '<div class="tip__container">' +
              ...
              "</div>" +
              '<div>' +
                "Exact date: " + d3.timeFormat("%A, %B %e, %Y")(d.date) + "<br>" + "Hours logged on this day: " + d.count.toFixed(2) +
              "</div>" +
            '</div>' 
          )
        }
      ...
      const generateGraph = () => {
        ...
        if ((fromDate == '') || (toDate == '')){ //if cond not needed
          ...
          for(var key in timeEntriesDict){
            if((Date.parse(fromDate) <= Date.parse(key.toString())) & (Date.parse(key.toString()) <= Date.parse(toDate))){
              timeEntryvalues.push({id: counter,date: d3.timeParse("%Y-%m-%d")(key.toString()),count: timeEntriesDict[key].time ,des:timeEntriesDict[key].des, isTangible: timeEntriesDict[key].isTangible,type: 'Entry'})
              if(timeEntriesDict[key].time > maxHoursCount){
                maxHoursCount = timeEntriesDict[key].time
              }
              counter += 1
