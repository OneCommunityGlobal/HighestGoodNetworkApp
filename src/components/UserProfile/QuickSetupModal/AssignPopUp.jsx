...
const AssignPopUp = props => {
  ...
  const setAssignedOnClick = () => {
    if (validation.volunteerAgree && googleDoc.length !== 0) {
      const data = {teams: [...userProfile.teams,title.teamAssiged], jobTitle: title.titleName, projects: [...userProfile.projects, title.projectAssigned], teamCode: title.teamCode}
      // remove duplicate
      ...
      setTitleOnSet(false);
      setValid(prev => ({volunteerAgree: false}));
      setIsOpen(false);
    }
  }
  const setNoOnClick = () => {
    setUserProfile(prev => ({
    ...prev, teams: [...userProfile.teams], jobTitle: userProfile.jobTitle, projects: [...userProfile.projects], teamCode: userProfile.teamCode
    }))
    setIsOpen(false)
  ...
  return (
    ...
    <Modal isOpen={isOpen} toggle={() => setIsOpen(false)} >
      ...
      <div className="">
        ...
        <Input type="checkbox" required id="agreement" value="true" onClick={() => checkboxOnClick()} />
        <Label for="agreement" className="">Volunteer Agreement Confirmed</Label>
        </div>
