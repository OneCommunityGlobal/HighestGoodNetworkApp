...
describe('TimeZoneDropDown Component', () => {
  ...
  it('renders with a selected time zone', () => {
    render(
      <Provider store={store}>
        <TimeZoneDropDown selected="Europe/London" onChange={mockOnChange} />
      </Provider>,
    );
  ...
  it('filters time zones based on filter prop', () => {
    render(
      <Provider store={store}>
        <TimeZoneDropDown filter="London" onChange={mockOnChange} />
      </Provider>,
    );
  ...
  it('calls onChange when a different time zone is selected', () => {
    render(
      <Provider store={store}>
        <TimeZoneDropDown onChange={mockOnChange} />
      </Provider>,
    );
  ...
  it('dispatches change event when filter prop changes', () => {
    const { rerender } = render(
      <Provider store={store}>
        <TimeZoneDropDown filter="" onChange={mockOnChange} />
      </Provider>,
    );
    rerender(
      <Provider store={store}>
        <TimeZoneDropDown filter="Tokyo" onChange={mockOnChange} />
      </Provider>,
    );
    expect(mockOnChange).toHaveBeenCalled();
  });
  ...
  test('handles invalid time zone names provided', () => {
    render(
      <Provider store={store}>
        <TimeZoneDropDown selected="InvalidTimeZone" />
      </Provider>,
    );
