...
import { userProfileMock, authMock, rolesMock, timeEntryMock, allTeamsMock, allProjectsMock, themeMock } from '../../../../__tests__/mockStates';
...
describe('<UserProfileEdit />', () => {
  ...
  it('renders the UserProfileEdit component', async () => {
    ...
    await waitFor(() => expect(mockProps.getUserProfile).toHaveBeenCalledWith('12345'));
  ...
  it('prevents submission if form validation fails', async () => {
    ...
    const saveChangesButton = saveChangesButtons.find(button => button.tagName === 'BUTTON');
  ...
  it('displays a warning message when unsaved changes are detected', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/You must click "Save Changes"/i)).toBeInTheDocument();
    });
  });
