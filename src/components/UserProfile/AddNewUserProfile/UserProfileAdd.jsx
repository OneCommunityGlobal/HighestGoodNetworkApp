...
class UserProfileAdd extends Component {
  ...
  onAssignTeam = assignedTeam => {
    this.setState(prevState => ({
      teams: [...prevState.teams, assignedTeam],
    }));
  };

  onAssignProject = assignedProject => {
    this.setState(prevState => ({
      projects: [...prevState.projects, assignedProject],
    }));
  };
  ...
  onClickGetTimeZone = () => {
    ...
    if (!location) {
      toast.alert('Please enter valid location');
      return;
    }
    ...
    axios
      ...
      .catch(err => {
        toast.error(`An error occurred : ${err.response?.data || err.message}`);
      });
  };
  ...
  handleLocation = e => {
    this.setState(prevState => ({
      userProfile: {
        ...prevState.userProfile,
        location: {
          ...prevState.userProfile.location,
