...
import { authMock, rolesMock, userProfileMock, allProjectsMock} from '../../../../__tests__/mockStates.js';
...
describe('AddUserProfile page structure', () => {
  ...
  instance = mountedUserProfileAdd.find(AddUserProfile).children().first().instance()
...
it('should call createUserProfile when create button is clicked', () => {
    ...
    const spy = jest.spyOn(mountedUserProfileAdd.find(AddUserProfile).children().first().instance(), 'createUserProfile');
  ...
  it('should call handleUserProfile when input is changed', () => {
    ...
    email.at(1)
    .simulate('change', {target: { value: expectedEmail, id: 'email' }});
    expect(instance.state.userProfile.email).toEqual(expectedEmail);
  });
