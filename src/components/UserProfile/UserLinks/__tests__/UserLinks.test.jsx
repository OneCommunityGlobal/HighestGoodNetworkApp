...
import { themeMock } from '../../../../__tests__/mockStates.js';
...
describe('Test suite for UserLinks components', () => {
  ...
  it('Test Case 2: Assert if the link name is not rendered when an empty string is passed', () => {
    links = [{ Link: '', Name: '' }];
    renderComponent(links);
    const userLinkContainer = screen.getByTestId('testLinkContainer');
    expect(userLinkContainer).toBeInTheDocument();
  ...
  it('Test Case 7: Assert if both the internal & external link Name is displayed in upper case', () => {
  ...
  it('Test Case 8: Assert if the component is able to render multiple links for a user', () => {
    links = [
      { Link: 'internalLink', Name: 'InternalLink' },
      { Link: 'http://externalLink.com', Name: 'ExternalLink' },
    ];
    renderComponent(links);
