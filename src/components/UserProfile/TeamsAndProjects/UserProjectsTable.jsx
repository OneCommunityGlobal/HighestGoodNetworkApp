...
const UserProjectsTable = React.memo(props => {
        ...
        return userProjects?.map(project => {
          const filteredTasks = sortedTasksByNumber.filter(task => {
            ...
            if (isTaskForProject) {
              ...
              if (situation === 'complete' && isCompletedTask) {
                return true;
              }
              if (situation === 'all') {
                return true;
  ...
  const deleteTasksTemporarily = projectId => {
    setFilteredTasks(filteredTasks?.filter(project => project._id !== projectId));
  };
                            ...
                            {!isCompletedTask && props.edit && props.role && canDeleteTasks && (
                              <td>
                                <Button
                                  ...
                                  onClick={() => removeOrAddTaskFromUser(task, 'remove')}
                                >
                                  Delete
                                </Button>
                              </td>
                            ...
                            {isCompletedTask && props.edit && props.role && (
                              <td>
                                <Button
                                  ...
                                  disabled={!canAssignProjectToUsers}
                                  onClick={() => removeOrAddTaskFromUser(task, 'add')}
                                >
                                  Add
                                </Button>
                              </td>
