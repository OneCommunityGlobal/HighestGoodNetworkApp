...
export class FaqButtonModal extends Component {
  state = {
    FaqButtonModalOpen:false,
    infoElements: [],
    fetchError: null,
    loading: true,
    editing: false,
  ...
  toggleFaqButtonModal = () => {
    this.setState({FaqButtonModalOpen: false});
  }
  ...
  render() {
    ...
    return (
    (CanRead)&&(
      // remove the icon "i" use button to word FAQ to replace it
      <div>
        <span
            style={{ cursor: 'pointer', color: 'white', textAlign: 'center'}}
            onClick={() => this.setState({ FaqButtonModalOpen: true })}
            title="Click for user class information"
          >
            FAQ
          </span>
        {FaqButtonModalOpen && (
          <Modal isOpen={FaqButtonModalOpen} toggle={this.FaqButtonModalOpen} size="lg">
...
export default connect(mapStateToProps, mapDispatchToProps)(FaqButtonModal);
