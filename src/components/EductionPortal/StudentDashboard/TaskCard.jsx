...
const TaskCard = ({ task, onMarkAsDone }) => {
  ...
  const progressPercentage = task.suggested_total_hours > 0 
    ...
    <div className={styles.taskCard}>
      ...
      <div className={styles.cardContent}>
        ...
        <div className={styles.timeInfo}>
          ...
          <span>{formatTime(task.logged_hours || 0)} • {formatDate(task.last_logged_date || task.created_at)}</span>
        </div>

        {/* Progress Bar */}
        <div className={styles.progressSection}>
          <div className={styles.progressInfo}>
            <span>Progress: {task.logged_hours || 0} / {task.suggested_total_hours} hrs ({progressPercentage}%)</span>
          </div>
