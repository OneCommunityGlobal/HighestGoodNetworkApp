...
import { Button, Modal, ModalHeader, ModalBody, ModalFooter, Container, Alert, Input, FormGroup } from 'reactstrap';
import { boxStyle } from 'styles';
import axios from 'axios';
import { ENDPOINTS } from '../../utils/URL';
import { authorizeWeeklySummaries, authorizeWeeklySummariesReportError } from "../../actions/weeklySummariesReportRecepients"
import { weeklySummaryRecipientsReducer } from "../../reducers/weeklySummaryRecipientsReducer"

export const PasswordInputModal = ({ onClose,
  open,
  checkForValidPwd,
  isValidPwd,
  setSummaryRecepientsPopup
}) => {
  const [state, dispatch] = useReducer(weeklySummaryRecipientsReducer, { passwordMatch: '', passwordMatchErr: '' })
  ...
  const authorizeWeeklySummariesButton = async () => {
    ...
    let url = ENDPOINTS.AUTHORIZE_WEEKLY_SUMMARY_REPORTS()
    try {
    ...
    <Container fluid>
      <Modal isOpen={open} toggle={onClose} autoFocus={false} size='lg'>
        ...
        <ModalBody style={{ textAlign: 'center' }}>
          {!isValidPwd && state.passwordMatchErr && <Alert color="danger">{state.passwordMatchErr}</Alert>}
          {isValidPwd && state.passwordMatch != '' && <Alert color="success">{state.passwordMatch} </Alert>}
          <FormGroup>
            <Input
              autoFocus
              type="password"
