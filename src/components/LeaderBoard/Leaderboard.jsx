...
function LeaderBoard({
  ...
  const calculateAdditionalWeeks = (from, till) => {
    const start = moment(from);
    const end = moment(till);
    const currentWeekStart = moment().startOf('week');
    let weeks = end.diff(start, 'weeks') + 1;
    if (start.isSameOrBefore(currentWeekStart) && end.isAfter(currentWeekStart)) {
      weeks -= 1; // Exclude the current week if the range includes it
    }
              ...
              <tr key={item.personId}>
                ...
                <td className="align-middle">
                  {allRequests &&
                    ...
                    allRequests[item.personId].map(request => {
                      const additionalWeeks = calculateAdditionalWeeks(
                        request.startingDate,
                        request.endingDate,
                      );
                      const isCurrentlyOff = moment().isBetween(
                        moment(request.startingDate),
                        moment(request.endingDate),
                        null,
                        '[]',
                      );
                      return (
                        <div>
                          ...
                          <span
                            ...
                            title="Additional weeks off beyond the current week"
                          >
                            {isCurrentlyOff && additionalWeeks > 0 && (
                              <sup> +{additionalWeeks}</sup>
                            )}
                          </span>
                        </div>
                      );
