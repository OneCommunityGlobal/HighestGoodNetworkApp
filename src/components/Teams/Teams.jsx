...
export function Team(props) {
  ...
  return (
    <tr className="teams__tr" id={`tr_${props.teamId}`}>
      ...
      <td>{props.name}</td>
      {/* Updated to replace interactive role on <td> with a <button> for accessibility */}
      <td className="teams__active--input" data-testid="active-marker">
        <button
          type="button" // Added explicit type attribute
          onClick={handleStatusClick}
          aria-label={`Set team ${props.active ? 'inactive' : 'active'}`} // Added aria-label for accessibility
          style={{
            border: 'none',
            background: 'none',
            padding: 0,
