  ...
  async componentDidMount() {
    ...
    data.forEach(element => {
      leaderboardData.push({
        didMeetWeeklyCommitment: element.totaltangibletime_hrs >= element.weeklyComittedHours ? true : false,
        name: element.name,
        ...
        intangibletimewidth: _.round(element.totalintangibletime_hrs * 100 / maxtotal, 0),
...
<div className="card hgn_leaderboard bg-dark text-center">
        ...
        <div>
            ...
            {leaderboardData.map(entry => 
                    ...
                    <tr key = {entry.personId} className ={entry.personId === loggedinUser ? "table-active row": "row"}>
                    <td className="col-1">
                    <i className="fa fa-circle" style={Object.assign(
                        {color: (() => entry.didMeetWeeklyCommitment? "green": "red")() } 
                    )} 
                    ...
                    <Link to ={`/profile/${entry.personId}`}>{entry.name}</Link>
                    </td>
                    ...
                    <div className="progress progress-leaderboard">
                                        ...
                                        })}            aria-valuenow= {entry.tangibletime}  aria-valuemin="0 " aria-valuemax= {maxtotal}
                                            data-toggle="tooltip" data-placement="bottom" title={`Tangible Effort: ${entry.tangibletime} hours`}>
                                            
                                        </div>
                                        <div className="progress-bar progress-bar-striped bg-info" role="progressbar " style={{width: entry.intangibletimewidth}}
                                            aria-valuenow={entry.intangibletime} aria-valuemin="0" aria-valuemax={maxtotal}
                                            data-toggle="tooltip" data-placement="bottom" title={`Intangible Effort: ${entry.intangibletime} hours`}>
                                     </div>
                                     </div>
