...
export const teamMemberTasksReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case "DELETE_TASK_NOTIFICATION_SUCCESS":
      return {...state, usersWithTasks: usersWithTasks.map(
        user => user.personId === action.payload.userId ? 
        {
          ...user, 
          tasks: user.tasks.map(task => task._id === action.payload.taskId ?
          {
            ...task, 
            taskNotifications: task.taskNotifications.filter(taskNotification => taskNotification._id !== action.payload.taskNotificationId)
          } : task)
        } : user)}
    default:
      return state;
