...
function Legend() {
  ...
  useEffect(() => {
    ...
    legend.onAdd = () => {
      ...
      let html = `<h4 style="margin-top:0;margin-bottom:5px;color:${darkMode ? 'white' : 'black'}">Project Status</h4>`;
      
      statuses.forEach((status, i) => {
        html += `
          <div style="display:flex;align-items:center;margin-bottom:5px;">
            <div style="width:12px;height:12px;border-radius:50%;background-color:${colors[i]};margin-right:8px;"></div>
...
function InteractiveMap() {
  ...
  const getStatusColor = status => {
    switch (status?.toLowerCase()) {
      case 'active': return '#DE6A6A';
      case 'delayed': return '#E3D270';
      case 'completed': return '#6ACFDE';
    ...
    <div style={styles.container}>
      ...
      <div style={styles.filterContainer}>
        <div style={styles.filterBox}>
          <h3 style={{ color: darkMode ? 'white' : 'black', marginTop: 0 }}>Filter Projects between Dates</h3>
        ...
        {loading ? (
          ...
          >
            ...
            {filteredOrgs.map((org, index) => (
              ...
              >
                <Tooltip 
                  className={darkMode ? 'dark-mode-tooltip' : ''}
                  permanent={false}
                >
