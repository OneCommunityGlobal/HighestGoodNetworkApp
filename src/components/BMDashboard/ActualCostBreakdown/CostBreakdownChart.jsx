...
function CostBreakdownChart() {
  ...
  const fetchCostData = async () => {
    ...
    setError(null);
    try {
      const url = `${ENDPOINTS.ACTUAL_COST_BREAKDOWN(
        selectedProject,
  ...
  const chartData = Object.entries(costData.current || {})
    .map(([key, value]) => ({
              ...
              {costData.percentageChange !== undefined && (
                <text
                  ...
                  className={`percentage-change ${
                    costData.percentageChange >= 0 ? 'positive' : 'negative'
                  }`}
                  style={{
                    fontSize: '16px',
                    fontWeight: 'bold',
                ...
                >
                  {`${costData.percentageChange >= 0 ? '+' : ''}${costData.percentageChange.toFixed(
                    1,
                  )}%`}
                  <tspan x="50%" dy="20" fontSize="12px">
                    vs prev period
                  </tspan>
                </text>
              )}
