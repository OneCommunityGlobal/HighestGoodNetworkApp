...
const NotificationBell = () => {
  ...
  // Extract the calculated values for time left
  const { hoursLeft, minutesLeft, daysLeft, remainingHours } = calculateTimeLeft;

  // Memoized calculation for hours left to work based on weekly committed hours
  const hoursLeftToWork = useMemo(() => {
    const remainingEffort = weeklycommittedHours - calculateTotalEffort;
    return remainingEffort > 0 ? remainingEffort.toFixed(2) : 0;
  }, [calculateTotalEffort, weeklycommittedHours]);
