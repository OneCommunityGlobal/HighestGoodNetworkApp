...
const JobFormBuilder = (props) => {
  ...
  const changeVisiblity=(event,field)=>{
    const updatedFields = formFields.map((item) =>
      item.questionText === field.questionText &&
      item.questionType === field.questionType
  ...
  const handleSubmit = (e) => {
    ...
    //   console.log(resolve.data);
    // })
  // WHAT IF THERE ARE MORE THAN ONE FORMAT????
    ...
    // TIME TO MAKE AN UPDATE TO THE FORM FORMAT.
    // to update a form first we need to create a job form.......
    axios.put(ENDPOINTS.UPDATE_JOB_FORM,{formId:"6753982566fcf3275f129eb4",title:jobTitle,questions:formFields,description:""}).then((resolve,reject)=>{
      console.log(resolve.data)
    })
    ...
    <div className="form-builder-container">
      <img src={OneCommunityImage} alt="One Community Logo" id="onecommunity-image" />
        <div className="jobform-navbar">
            ...
            {jobPositions.map((e,i)=>(<option key={i} value={e}>{e}</option>))}
          </select>
            </div>
        </div>
      <h1>FORM CREATION</h1>
          
     {role==='Owner'? <div className="custom-form">
      <p>Fill the form with questions about a specific position you want to create an ad for. The default questions will automatically appear and are alredy selected. You can pick and choose them with the checkbox.</p>
      <form>
        {formFields.map((field, index) => (
            <div className="form-div" key={index}>
                <input type="checkbox" id="form-div-checkbox" checked={field.visible} onChange={(event)=>changeVisiblity(event,field)}/>
            <div key={index} className="form-field">
                <label className="field-label">{field.questionText}</label>
            <div className="field-options">
                {field.questionType === "textbox" && <input type="text" placeholder="Enter Text here"/>}
                {field.questionType === "date" && <input type="date" placeholder="Enter date" />}
