...
const StatCard = ({ label, value, icon, bgColor, darkMode }) => (
...
const CalendarSection = ({ title, events, type, icon, darkMode }) => {
  const getButtonText = () => {
    switch (type) {
      case 'seeding':
        return '+ Add Seeding';
  ...
  return (
    <div className={styles.calendarCard}>
      ...
      <div className={styles.eventsContainer}>
        {events.map(event => (
          <CalendarEventCard key={event.id} event={event} type={type} darkMode={darkMode} />
        ))}
      </div>
    </div>
  );
      ...
      <main className={styles.main}>
        ...
        <div className={styles.statsGrid}>
          ...
          <StatCard
            ...
            value="12"
            icon="🌿"
            bgColor={darkMode ? '#355c2d' : '#f1f8e9'}
            darkMode={darkMode}
          />
          <StatCard
            ...
            value="4"
            icon="🌾"
            bgColor={darkMode ? '#5d4037' : '#fff3e0'}
            darkMode={darkMode}
          />
          <StatCard
            label="Seed Orders"
