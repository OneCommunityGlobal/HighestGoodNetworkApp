...
class RenderInfringment extends Form {
  ...
  render() {
    const { date, description } = this.state.data;
    const { isUserAdmin, index } = this.state;
    const { handleInfringment } = this.props;
    const isEmpty = !(!!date && !!description);
    const className = isEmpty ? "fa fa-square" : "fa fa-square infringement";
            ...
            <div className="row ml-1">
              {!isEmpty && (
                <span
                  ...
                  onClick={() =>
                    handleInfringment(this.state.data, "delete", index)
                  }
                />
              )}
            </div>
