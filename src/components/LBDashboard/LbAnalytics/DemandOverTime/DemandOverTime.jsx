...
import { DatePicker, Select, Radio, Card, Row, Col, Typography, Divider } from 'antd';
    ...
    <div className={styles.container}>
      <Card className={styles.filterCard}>
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={6}>
            <div className={styles.filterItem}>
              <Text strong>Listing/Bidding:</Text>
              <Select
                value={listingType}
            ...
            <div className={styles.filterItem}>
              ...
              <Select value={metric} onChange={handleMetricChange} className={styles.select}>
                <Option value="pageVisits" disabled={false}>
                  <Text className={styles.metricCategory}>DEMAND:</Text> Page Visits
                </Option>
                <Option value="numberOfBids" disabled={isBidMetricDisabled}>
                  <Text className={styles.metricCategory}>DEMAND:</Text> Number of Bids
                </Option>
                <Option value="averageRating" disabled={false}>
                  <Text className={styles.metricCategory}>DEMAND:</Text> Average Rating
                </Option>
                <Option value="averageBid" disabled={isBidMetricDisabled}>
                  <Text className={styles.metricCategory}>REVENUE:</Text> Average Bid
                </Option>
                <Option value="finalPrice" disabled={isBidMetricDisabled}>
                  <Text className={styles.metricCategory}>REVENUE:</Text> Final Price/Income
                </Option>
                <Option value="occupancyRate" disabled={false}>
                  <Text className={styles.metricCategory}>VACANCY:</Text> Occupancy Rate
                </Option>
                <Option value="averageDuration" disabled={false}>
                  <Text className={styles.metricCategory}>VACANCY:</Text> Average Duration of Stay
                </Option>
              </Select>
            </div>
          </Col>
      ...
      <div className={styles.chartContainer}>
        {compareType === 'villages' ? (
          <VillageDemandChart data={data} metric={metric} dateRange={dateRange} />
        ) : (
          <PropertyDemandChart data={data} metric={metric} dateRange={dateRange} />
        )}
      </div>
    </div>
  );
