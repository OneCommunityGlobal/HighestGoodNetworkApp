...
function PinterestPost() {
  ...
  useEffect(() => {
    async function fetchScheduleList() {
      try {
        const scheduledPins = (await axios.get(ENDPOINTS.SCHEDULE_PINTEREST)).data;
        setScheduleList(scheduledPins);
      } catch (err) {
  ...
  async function handlePost(event) {
    ...
    const postValue = await getPostValue();
  ...
  const sendSchedule = async () => {
    const postData = await getPostValue();
    try {
  ...
  function scheduleCard(pinTitle, pinDescription, image, time, id) {
    const pinScheduledTime = new Date(time);
    ...
    return (
      <div className="schedule-card" key={id}>
        ...
        />
        <div className="schedule-title">{pinTitle}</div>
        <div className="schedule-time">
          {pinScheduledTime.toLocaleDateString()} &nbsp; {pinScheduledTime.toLocaleTimeString()}
        </div>
        <div className="schedule-description">{pinDescription}</div>
        <img src={image} alt="" />
      </div>
    );
  }
      ...
      <div className="pinterest-main">
        ...
        <div className="pinterest-schedule">
          ...
          {scheduleList.map(item => {
            ...
            const image = postData.media_source.url
              ? postData.media_source.url
              : `data:image/png;base64,${postData.media_source.data}`;
            const id = item._id;
            return scheduleCard(
              postData.title,
