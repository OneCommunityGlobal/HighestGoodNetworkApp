...
function PinterestPinComposer({ network }) {
  ...
  // Pin needs an image to be published to Pinterest API
  ...
  const queueHasDraft = queuedDraftText.trim().length > 0;
  ...
  // Always re-fetches from backend to get real MongoDB _ids
  ...
  const getUnfilledRequiredFields = () => {
    ...
    if (!cleanBoard) missing.push('Board');
  ...
  // Publish immediately via POST /api/pinterest/createPin
  ...
  const handleQueueDateChange = evt => {
    ...
    applyQueueDateTime(evt.target.value, queueTime);
  };

  const handleQueueTimeChange = evt => {
    if (!evt.target.value) return;
    applyQueueDateTime(queueDate, evt.target.value);
  };
  ...
  // ── Queue save ─────────────────────────────────────────────────────────────
  // savePinToBackend calls POST /api/pinterest/schedule (no id returned).
  // We always re-fetch after save to get real MongoDB _ids for delete to work.
  ...
  const handlePersistQueueEntry = async () => {
    ...
    setIsSyncing(true);
    try {
      await savePinToBackend(queueRecord);
      // Re-fetch to get real MongoDB _ids — local ids are not reliable for delete
  ...
  // ── Queue edit ─────────────────────────────────────────────────────────────
  // Note: backend has no PUT /schedule route, so editing creates a new entry
  ...
  const handleLoadQueueEntry = entryId => {
    ...
    // Re-populate compose fields from the stored postData title
  ...
  // ── Queue delete ───────────────────────────────────────────────────────────
  // Uses the real MongoDB _id returned by fetchSavedPins normalisation.
      ...
      <div className={classNames(pStyles['pin-panetabs'], { [pStyles.dark]: isDark })}>
        {paneTabs.map(({ id, label }) => (
          <button
            key={id}
            type="button"
            className={classNames(pStyles['pin-panetab'], {
              [pStyles.active]: activePane === id,
            })}
      ...
      {activePane === 'compose' ? (
          ...
          <div className={pStyles['pin-grid']}>
            ...
            <div className={classNames(pStyles['pin-card'], { [pStyles.invalid]: warnBoard })}>
              <div className={pStyles['pin-field__header']}>
                ...
                <span
                  className={classNames(pStyles['pin-field__meta'], {
                    [pStyles.invalid]: warnBoard,
                  })}
                >
            ...
            >
              ...
              {warnDesc && (
                <p className={pStyles['pin-field__error']}>
                  Description exceeds Pinterest&apos;s {PIN_DESC_MAX.toLocaleString()}-character
                  limit.
                </p>
              )}
              <div style={pinFieldRow}>
            ...
            <div className={pStyles['pin-preview__header']}>
              ...
              <div className={pStyles['pin-preview__actions']}>
                ...
                <button
                  type="button"
                  style={{
                    ...pinButtonStyle('outline', isDark),
                    opacity: canCopyDraft ? 1 : 0.5,
                  }}
        ...
        <div className={pStyles['pin-queue__grid']}>
          ...
          <section className={classNames(pStyles['pin-card'], pStyles['pin-card--queue-editor'])}>
            <h3>Pin Queue</h3>
            <p>
              Pick a date and time, then save this pin for the scheduler to publish automatically.
            </p>
            {activeQueueEntry && (
              <p className={pStyles['pin-queue__edit-note']}>
                Editing &quot;{activeQueueEntry.pinTitle || 'Untitled pin'}&quot;. Saving will add a
                new entry — delete the old one if no longer needed.
              </p>
            )}
            ...
            <div className={pStyles['pin-queue__action-row']}>
              <button
                ...
                disabled={!queueHasDraft || isSyncing}
          ...
          <section className={classNames(pStyles['pin-card'], pStyles['pin-card--queue-list'])}>
            ...
            <p className={pStyles['pin-field__hint']}>
              Pins are published automatically by the scheduler and removed from this list once
              posted.
            </p>
            {isLoadingQueue ? (
