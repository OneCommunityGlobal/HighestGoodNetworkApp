      ...
      if (response.data.success) {
         if (editingJobId) {
          try {
            ...
            toast.error('Failed to delete old post');
          }
          setEditingJobId(null); // Reset after delete
         }
        if (scheduleDate && scheduleTime) {
      ...
      <div className="email-update-container">
        <div className="editor">
          ...
          <div className={`${darkMode ? 'bg-oxford-blue text-light' : ''}`} style={{ minHeight: '100%' }}>
            ...
            <div className="mt-6">
  <h4 className={`text-md font-semibold mb-2 ${darkMode ? 'text-white' : 'text-black'}`}>Scheduled Posts</h4>
  {scheduledPosts.length === 0 ? (
    <p className={`${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>No posts scheduled yet.</p>
  ) : (
    <ul className="space-y-4">
      {scheduledPosts.map((post) => (
        <li key={post.jobId} className="border p-4 rounded-md bg-white dark:bg-gray-800">
  ...
  <div className="flex gap-2 mt-2">
    <button
      ...
      onClick={() => {
        ...
        const dt = new Date(post.scheduleTime);
