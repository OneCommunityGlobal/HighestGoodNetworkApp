...
const SummaryBar = React.forwardRef((props, ref) => {
  ...
  const { displayUserId, toggleSubmitForm, summaryBarData } = props;

  // Use Redux hooks instead of connect
  const dispatch = useDispatch();
  const authUser = useSelector(state => state.auth.user);
  const displayUserProfile = useSelector(state => state.userProfile);
  const displayUserTask = useSelector(state => state.userTask);
  ...
  const authId = authUser.userid;
  const isAuthUser = displayUserId === authId;
  const initialInfo = { in: false, information: '' };

  // Use useMemo for computed values to avoid recalculation on every render
  const committedHours = useMemo(() => displayUserProfile?.weeklycommittedHours ?? 0, [
    displayUserProfile?.weeklycommittedHours,
  ]);

  const missedHours = useMemo(() => displayUserProfile?.missedHours ?? 0, [
    displayUserProfile?.missedHours,
  ]);

  const weeklyCommittedHours = useMemo(() => committedHours + missedHours, [
    committedHours,
    missedHours,
  ]);
  ...
  // Add a cache reference to track last fetch time for API calls
  const lastFetchRef = useRef({});

  // Helper function to check if we should fetch data (cache control)
  const shouldFetchData = useCallback(id => {
    const now = Date.now();
    const lastFetch = lastFetchRef.current[id];
    const cacheTimeout = 60000; // 1 minute cache

    return !lastFetch || now - lastFetch > cacheTimeout;
  }, []);

  // Memoized function to load user profile with cache check
  ...
  // Memoized function to load user tasks with cache check
  const loadUserTasks = useCallback(async () => {
    if (!displayUserId) return;

    if (shouldFetchData(`tasks_${displayUserId}`)) {
      try {
        // Use Redux action instead of direct axios call
  ...
  // Memoized getter functions for badge/infringement counts
  const getInfringements = useCallback(() => {
    return displayUserProfile?.infringements?.length || 0;
  }, [displayUserProfile?.infringements]);
  ...
  const getBadges = useCallback(() => {
    ...
    return displayUserProfile?.badgeCollection.reduce((total, badge) => {
      ...
      return total + Math.round(Number(badge?.count || 0));
    }, 0);
  }, [displayUserProfile?.badgeCollection]);
  ...
  // Add new text field or suggestion category
  const editField = async event => {
    event.preventDefault();
    const data = readFormData('newFieldForm');
    if (data) {
  ...
  // Memoized function to open suggestion modal
  const openSuggestionModal = useCallback(async () => {
    if (!showSuggestionModal) {
      try {
        const res = await httpService.get(
          `${ApiEndpoint}/dashboard/suggestionoption/${displayUserProfile?._id}`,
  ...
  // Memoized function to get weekly summary
  const getWeeklySummary = useCallback(user => {
    const latestSummary = user?.weeklySummaries?.[0];
    return latestSummary && new Date() < new Date(latestSummary.dueDate)
      ? latestSummary.summary
      : '';
  ...
  }, [isAuthUser, authUser, displayUserTask.length, loadUserProfile, loadUserTasks]);
  ...
  }, [displayUserProfile, summaryBarData, getBadges, getInfringements, getWeeklySummary]);

  // Check if we should open the suggestions modal on mount
  useEffect(() => {
    const shouldOpenSuggestions = localStorage.getItem('openSuggestionsModal');
    if (shouldOpenSuggestions === 'true') {
      localStorage.removeItem('openSuggestionsModal'); // Clear the flag
  ...
  // Memoized function to get container class
  const getContainerClass = useCallback(() => {
    ...
    return darkMode
      ? 'bg-space-cadet disabled-bar text-light box-shadow-dark'
      : 'bg--bar disabled-bar text--black box-shadow-light';
  }, [isAuthUser, canEditData, darkMode]);
  ...
  // Get placeholder text based on form state
  const getPlaceholderText = () => {
    if (extraFieldForSuggestionForm === 'suggestion') {
      return editType === 'delete'
        ? 'write the category number, like 1 or 2 etc'
            ...
            >
              <div className="m-auto p-2 text-center">
                <span
                  ...
                  tabIndex={0}
                  onClick={toggleSubmitForm}
                  onKeyDown={e => (e.key === 'Enter' || e.key === ' ') && toggleSubmitForm()}
                  className="summary-toggle"
                  style={{ cursor: 'pointer', fontSize: '1.1rem' }}
                >
                  {renderSummaryMessage()}
