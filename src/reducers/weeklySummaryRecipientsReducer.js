...
export default function weeklySummaryRecipientsReducer(state = initialState, action) {
  switch (action.type) {
    case actions.DELETE_WEEKLY_SUMMARIES_RECIPIENTS:
      return {
        ...state,
        recepientsArr: state.recepientsArr.filter(
          (elem) => elem._id !== action.payload.userid
        ),
      };
