...
describe('reset password popup', () => {
  ...
  describe('Behavior', () => {
    it('should change value while the user type in the textbox ', async () => {
      await userEvent.type(screen.getByLabelText(/confirm password/i), 'test', { allAtOnce: false });
    ...
    it('should popup error when the password does not meet the requirement', async () => {
      await userEvent.type(screen.getByLabelText(/new password/i), 'AB@12345!', { allAtOnce: false });
      await userEvent.type(screen.getByLabelText(/confirm password/i), 'AB@12345!', { allAtOnce: false });
