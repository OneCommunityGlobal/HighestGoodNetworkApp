      ...
      try {
        ...
        if (requestId && pendingRequests[requestId]) {
          ...
          console.error(
            ...
            `Duration: ${duration.toFixed(2)}ms | Status: ${error.response?.status || 'unknown'} | ` +
            `From: ${request.caller.component} (${request.caller.file})`,
...
axios.interceptors.request.use(request => {
  try {
    ...
    console.log(`[API CALL] ${request.method?.toUpperCase() || 'GET'} ${request.url}`, request.data || '');
  }
